...
function love.load()
    ...
    font = love.graphics.setNewFont(32, "normal", 1)
...
function love.draw()
    ...
    if game_over then    
        love.graphics.print( {{1,0.2,0.2,1},"GAME OVER ", {1,1,1,1}, "Score: " .. player.score}, font, window.width/2-180, window.height/2-180)
    else    
        ...
        -- score
        love.graphics.print( {{1,1,1,1}, "Score: " .. player.score}, font, 2)
    end
end
...
function move_asteroids(dt)
    if table_length(asteroids) < NUMBER_OF_ASTEROIDS then
        ...
        table.insert(asteroids,{
            ...
            vx = coefs[math.random(1,2)]*ASTEROID_MAX_SPEED*math.random(40,100)/100, vy = coefs[math.random(1,2)]*ASTEROID_MAX_SPEED*math.random(70,100)/100,
